...
class MySqlDdlRoundTripTest {

    @Container
    @SuppressWarnings("resource")
    static final MySQLContainer<?> CONTAINER = new MySQLContainer<>("mysql:8.4")
            // schema == database in MySQL; provision under the test's name.
            .withDatabaseName("RT_TEST").withUsername("rt").withPassword("rt");
    ...
    void full_round_trip_with_per_step_database_service_verification() throws Exception {
        ...
        // CREATE
        executeAndVerify(dialect.ddlGenerator().createSchema(SCHEMA.name(), true), info -> {
        ...
        // Cross-table UPDATE — MySQL UPDATE … JOIN syntax (same as MariaDB).
        String qC = dialect.quoteIdentifier(SCHEMA.name(), CUSTOMERS.name());
        String qO = dialect.quoteIdentifier(SCHEMA.name(), ORDERS.name());
        try (PreparedStatement ps = connection.prepareStatement("UPDATE " + qC + " C JOIN (" + "  SELECT "
                + dialect.quoteIdentifier("CUSTOMER_ID") + ", " + "         MAX(" + dialect.quoteIdentifier("NOTE")
                + ") AS NEW_NAME" + "  FROM " + qO + "  GROUP BY " + dialect.quoteIdentifier("CUSTOMER_ID")
                + ") O ON O." + dialect.quoteIdentifier("CUSTOMER_ID") + " = C." + dialect.quoteIdentifier("ID")
                + " SET C." + dialect.quoteIdentifier("NAME") + " = O.NEW_NAME")) {
        ...
        // Don't DROP SCHEMA — the container's user only owns RT_TEST.
    }
